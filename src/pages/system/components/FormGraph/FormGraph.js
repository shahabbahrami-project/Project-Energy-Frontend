...
const FormGraph = (props) => {
  const classes = useStyles();

  const handleDateChangeFrom = (date) => {
    try {
      const dateformat = date.toISOString();
      const DateFromNew = { ...props.sensorDataFilter, startDate: dateformat };
      props.setSensorDataFilter(DateFromNew);
    }
    catch (err) {
      const DateFromNew = { ...props.sensorDataFilter, startDate: null };
      props.setSensorDataFilter(DateFromNew);
    }
  ...
  const handleDateChangeTo = (date) => {
    try {
      const dateformat = date.toISOString();
      const DateToNew = { ...props.sensorDataFilter, endDate: dateformat };
      props.setSensorDataFilter(DateToNew);
    }
    catch (err) {

      const DateToNew = { ...props.sensorDataFilter, endDate: null};
      props.setSensorDataFilter(DateToNew);
    }
        ...
        <FormControl className={classes.textField0} variant="outlined">
          <MuiPickersUtilsProvider utils={DateFnsUtils} className={classes.datePickerText}>
            <KeyboardDatePicker
              ...
              id="date-picker-inline"
              label="From"
              value={props.sensorDataFilter.startDate}
              onChange={handleDateChangeFrom}
              KeyboardButtonProps={{
                'aria-label': 'change date',
              }}
        ...
        <FormControl className={classes.textField0} variant="outlined">
          <MuiPickersUtilsProvider utils={DateFnsUtils} className={classes.datePickerText}>
            <KeyboardDatePicker
              ...
              id="date-picker-inline"
              label="To"
              value={props.sensorDataFilter.endDate}
